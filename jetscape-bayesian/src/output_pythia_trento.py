...
#   Prepare output file
    ...
    header += '#       trento-stat          |      pythia-truth                 | slowjet  \n'
    header += '#iev mult xgj quench b Np e2   part1 pT eta phi part2 pT eta phi   Njets pT eta phi pT eta phi\n'
    ...
    for i in range(trento_seed,trento_seed+nevt): 
        ...
        #   Check for gamma in [5] position, for QCD events quarkjet is always [5]
        ...
        #   Rescale daughters by momentum, not same as energy
            ...
            for j in daughters:
                ...
                prt.e(prt_e)
#
#  The Trento particle production below is un-physical, put together by last year's summer student.
...
#       Prepare output, stats include trento information (values set to zero if not turned on)
        stats_output = '{: 5d}'.format(i) + '{: 6d}'.format(int(mult[i])) + '{: 5.3f}'.format(xgj) + '{: 4.2f}'.format(quench)
        stats_output += '{: 6.2f}'.format(b[i]) + '{: 4d}'.format(int(Npart[i])) +  '{: 5.3f}'.format(e2[i])
            
        if (pythia_on):          
            gamma_output = '{: 3d}'.format(gammajet.id()) + '{: 5.3f}'.format(gammajet.pT()) + '{: 8.3f}'.format(gammajet.eta()) + '{: 8.3f}'.format(gammajet.phi())
            quark_output = '{: 3d}'.format(quarkjet.id()) + '{: 5.3f}'.format(quarkjet.pT()) + '{: 8.3f}'.format(quarkjet.eta()) + '{: 8.3f}'.format(quarkjet.phi())        
        ...
        if Njets>0:
            slowjet_output += ' {: 5.3f}'.format(slowJet.pT(0)) + ' {: 5.3f} '.format(slowJet.p(0).eta()) + ' {: 5.3f}'.format(slowJet.phi(0))
            if Njets>1:
                slowjet_output += ' {: 5.3f}'.format(slowJet.pT(1)) + ' {: 5.3f} '.format(slowJet.p(1).eta()) + ' {: 5.3f}'.format(slowJet.phi(1))
            else:
                slowjet_output += ' 0 0 0'
        else:
            slowjet_output += ' 0 0 0 0 0 0'
            
        output = stats_output + ' ' + pythia_output + ' ' +  slowjet_output + '\n'
        f.write(output)
